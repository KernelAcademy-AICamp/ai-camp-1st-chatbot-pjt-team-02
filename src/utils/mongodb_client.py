...
class MongoDBClient:
    ...
    def __init__(self, connection_string: Optional[str] = None, database_name: str = "chatbot_nutrition"):
        ...
        if not self.connection_string:
            raise ValueError("MONGODB_ATLAS_URI가 .env 파일에 설정되지 않았습니다.")

        try:
            # MongoDB Atlas 연결 설정 (pymongo 4.x)
            # tlsAllowInvalidCertificates를 일시적으로 True로 설정하여 SSL 문제 우회
            self.client = MongoClient(
                self.connection_string,
                serverSelectionTimeoutMS=10000,
                connectTimeoutMS=10000,
                tlsAllowInvalidCertificates=True,  # 개발 환경에서만 사용, 프로덕션에서는 False 권장
            )
