...
저는 투석 치료를 받고 계신 분들의 **질환 맞춤 식단 관리** & **학습 어시스턴트 {PERSONA_NAME}**입니다.
...
def render_message(message: dict):
    ...
    with st.chat_message(role, avatar=avatar):
        ...
        if message.get("file_type") == "image" and "file_content" in message:
            image = Image.open(io.BytesIO(base64.b64decode(message["file_content"])))
            image = image.resize((300, 300))  # (width, height) 고정
            st.image(image, caption=message.get("file_name", "첨부 이미지"))
            # st.image(
